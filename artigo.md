...
### 3.5 Confirmação em subgenoma real de *U. brizantha* — assembly de *U. decumbens* cv. Basilisk

**Contexto**: não existe assembly próprio de *U. brizantha* (segue "órfã genômica"), mas existe algo melhor que o proxy usado até aqui (*U. ruziziensis*, só parente próximo): o assembly haplótipo-resolvido, cromossomo-completo de **_Urochloa decumbens_ cv. Basilisk** (`GCA_964030465.3`, de Vega Group/Earlham Institute, ENA, BioProject `PRJEB73762`; bioRxiv 10.1101/2024.09.25.614935, PMC12005165) é um alotetraploide cujos 36 cromossomos foram rotulados por origem de subgenoma, confirmado no próprio texto do artigo: sufixo **"b" = subgenoma derivado de *U. brizantha* diploide**; sufixo **"rd" = subgenoma derivado de *U. ruziziensis*/*U. decumbens* diploide**. Ou seja, parte desse assembly é sequência real de *U. brizantha*, com anotação gênica completa (~126 mil genes preditos).

**Metodologia**: BLASTP local (BLAST+ 2.17.0) da proteína de `locus_001` contra o proteoma anotado de *U. decumbens* (150.710 proteínas), seguido de cruzamento dos hits contra a tabela de features para identificar o subgenoma de origem (script/log completos em `brizantha_search/README.md`).

**Resultado**: 5 cópias quase idênticas (99,4–100% identidade), refletindo a redundância de haplótipos do assembly — 2 em cromossomos "rd", 1 em cromossomo "b" (`CAL5051155.1`, cromossomo `36b`, gene `URODEC1_LOCUS91967`, OZ075146.1:7.157.787–7.159.709, fita +), 2 em scaffolds não localizados. **`CAL5051155.1` é a cópia real do subgenoma *brizantha*** e difere da predição de `locus_001` (via *U. ruziziensis*) por **exatamente 1 aminoácido** (posição ~53, V→I) — 173/174 aa idênticos.

**Dois ganhos diretos sobre os resultados anteriores**:
1. **Divergência espécie-proxy agora quantificada**: não é mais uma incógnita (ver §7.2) — é 1 substituição conservativa em 174 posições entre a predição via *U. ruziziensis* e a sequência real do subgenoma *brizantha* de um genoma publicado.
2. **UTRs reais disponíveis pela primeira vez**: o gene `URODEC1_LOCUS91967` tem anotação de mRNA completo (1.222 nt) vs. CDS (525 nt) — ~697 nt de UTR real, algo que a predição em *U. ruziziensis* via miniprot nunca teve (miniprot só prediz CDS). Estrutura gênica de 4 éxons/3 íntrons confirmada, idêntica à contagem predita para `locus_001` — validação cruzada da estrutura gênica por uma fonte totalmente independente.

**O que isso NÃO resolve** (ver §7.1–7.2 revisados): a cv. Basilisk (*U. decumbens*) não é a cv. Marandu (*U. brizantha*, a cultivar real do projeto) — pode haver diferença adicional entre acessos/cultivares de *brizantha* não capturada aqui. Confirmação por PCR+Sanger em DNA genômico real da cv. Marandu continua recomendada antes do desenho final de primers/dsRNA, mas agora com uma sequência-molde de *brizantha* de verdade (não mais só *U. ruziziensis*) e com o risco de divergência já quantificado como baixo.

Sequências e detalhes completos em `brizantha_search/` (`brizantha_CDS.fasta`, `brizantha_mRNA_with_UTR.fasta`, `locus_001_vs_decumbens.tsv`).

### 3.6 Correlação locus_XXX (pipeline) → gene ID real no subgenoma *brizantha*, para as 6 loci confirmadas

**Motivação**: até aqui, a confirmação em subgenoma real (§3.5) tinha sido feita só para `locus_001`. Estendi a mesma metodologia (BLASTP local contra o proteoma anotado de *U. decumbens* cv. Basilisk, cruzado com o mapeamento cromossomo→subgenoma) para as outras 5 loci que também confirmam `pebp_confirmed=True` e `rbh_confirmed=True` no pipeline (`locus_002`, `005`, `009`, `012`, `015`), pra ter um "ID de gene real" correlacionado com cada `locus_XXX` interno, não só com o candidato principal. **Nota importante verificada antes de fazer essa busca**: o assembly *U. ruziziensis* usado como alvo do pipeline (`Embrapa_Uruz_1.0`) não tem anotação gênica oficial no NCBI (confirmado via API `dataset_report`, sem campo `annotation_info`) — não existe, portanto, um "gene ID NCBI" nativo para nenhum `locus_XXX` na espécie-proxy; o único ID de gene real disponível vem da correlação com o subgenoma *brizantha* do assembly de *decumbens*, como já feito para `locus_001`.

Tabela completa em `brizantha_search/loci_brizantha_correlation.tsv`; resumo:

| locus (pipeline) | classificação filogenética | melhor cópia subgenoma **b** (brizantha) | identidade | gene ID (locus_tag) |
|---|---|---|---|---|
...
| `locus_012` | **MFT-like** (não é FT-like, ver §3.4/§4) | `CAL4950975.1` | 100,0% | `URODEC1_LOCUS38694` (chr 17b) |
| `locus_015` | FT-like (par 009/015) | `CAL4952393.1` | 98,8% | `URODEC1_LOCUS39484` (chr 17b) |

**Achados notáveis desta extensão**:
- `locus_001` segue sendo o caso mais limpo: a cópia do subgenoma **b** já é, simultaneamente, o melhor hit *geral* de todo o proteoma decumbens (nenhuma cópia rd é mais próxima). Nos outros 5 loci, isso **não** se repete — o melhor hit geral cai numa cópia **rd**, e a cópia **b** correspondente tem identidade igual ou, no caso de `locus_005`, sensivelmente menor (91,6% vs. 100% da cópia rd) — divergência maior que qualquer uma das outras 5 loci confirmadas.
- `locus_009` e `locus_015` (parálogos quase idênticos já identificados na filogenia, branch length ≈0 entre si) convergem para exatamente a mesma cópia b (`CAL4952393.1`/`URODEC1_LOCUS39484`) — replicação independente, por um método totalmente diferente (BLASTP contra outro assembly), do sinal de parologia recente já visto na árvore.
- `locus_012` entra na tabela só por rastreabilidade de ID — já está reclassificado como MFT-like (§3.4/§4), não é candidato ao gene FT-like da proposta.
- `locus_002` é o caso mais frágil: fragmento curto (76 aa, só evidência TBLASTN, nunca confirmado por miniprot) com 3 cópias empatadas em 100% de identidade no proteoma decumbens (1 rd + 2 b) — o nível de evidência não permite apontar uma única cópia b "correta".

**Interpretação**: a divergência maior de `locus_005` no subgenoma b (91,6%) é um dado novo que deveria ser considerado se esse locus for cogitado como alvo alternativo/complementar de dsRNA — a região pode ter menos identidade com a sequência real de *brizantha* do que a similaridade observada no proxy *ruziziensis* sugeria.
...
### 7.2 Gap espécie-proxy — PARCIALMENTE RESOLVIDO em 2026-07-11 (era bloqueante)

`locus_001` foi originalmente encontrado só em *U. ruziziensis* (diploide), não em *U. brizantha* (poliploide agâmica, a espécie real de bancada), sem nenhuma estimativa de divergência entre as duas. **Atualização (§3.5)**: o subgenoma *brizantha* real do assembly de *U. decumbens* cv. Basilisk (`GCA_964030465.3`) tem uma cópia quase idêntica (`CAL5051155.1`, cromossomo `36b`) — **99,4% idêntica, 1 substituição de aminoácido em 174**. A divergência deixou de ser uma incógnita e passou a ser um número baixo e conhecido.

**O que ainda falta**: essa é a cultivar Basilisk, não a Marandu (cultivar real do projeto) — pode haver diferença adicional entre cultivares/acessos de *brizantha* não capturada. **Recomendação prática atualizada**: usar `brizantha_search/brizantha_CDS.fasta` (não mais a predição em *ruziziensis* isolada) como molde para desenho de primers de triagem; confirmar por PCR+Sanger em DNA genômico real da cv. Marandu (§7.1) antes do desenho final — mas agora partindo de uma base de *brizantha* de verdade, com risco de divergência residual já conhecido como baixo pela comparação entre cultivares/subgenoma.
...
### 7.5 Extensão exata do transcrito — PARCIALMENTE RESOLVIDO em 2026-07-11 (era: sem UTRs, sem RNA-seq real)

O modelo gênico original de `locus_001` (4 éxons/3 íntrons, stop códon no lugar certo) veio inteiramente de alinhamento spliced-aware via miniprot, sem UTR modelada. **Atualização (§3.5)**: o gene homólogo real no subgenoma *brizantha* do assembly *U. decumbens* (`URODEC1_LOCUS91967`) tem anotação de **mRNA completo com UTRs** (1.222 nt totais, 525 nt de CDS, ~697 nt de UTR 5'+3' reais) — disponível em `brizantha_search/brizantha_mRNA_with_UTR.fasta`. A estrutura de 4 éxons/3 íntrons foi confirmada de forma independente, reforçando a confiança na predição original.

**O que ainda falta**: não está confirmado se a anotação de UTR desse assembly veio de evidência transcricional direta (RNA-seq) da própria *U. decumbens*/*brizantha* ou de modelo computacional — não verificado nesta análise. **Recomendação prática atualizada**: usar `brizantha_mRNA_with_UTR.fasta` como ponto de partida para desenho de primers/dsRNA na região de UTR 3' (mais específica entre parálogos), mas confirmar por 5'/3' RACE ou RT-PCR ancorado em RNA real de *U. brizantha* cv. Marandu antes do desenho final — o risco caiu de "nenhuma informação de UTR" para "UTR candidata de fonte externa a confirmar".
...
`locus_001` é um candidato forte e bem fundamentado **dentro do que a bioinformática pode provar**, e ficou mais forte ainda em 2026-07-11 com a confirmação em sequência real de *brizantha* (§3.5): divergência de apenas 1 aa e UTRs reais disponíveis, reduzindo dois dos gargalos (§7.2, §7.5) de "bloqueante" para "parcialmente resolvido". Mas isso ainda não é o mesmo que "pronto para desenho final de primers/dsRNA em bancada". **Seguem bloqueantes**: §7.1 (validação experimental — nenhum PCR/RT-PCR real feito ainda) e §7.3 (off-target — ainda não avaliado, exigido pela proposta). Recomendação de sequência: PCR+Sanger em DNA genômico real da cv. Marandu usando `brizantha_search/brizantha_CDS.fasta` como molde primeiro, depois off-target, antes de qualquer decisão de síntese de dsRNA.
...
| 2026-07-11 | **Confirmação em subgenoma real de *U. brizantha* (§3.5, novo).** *U. brizantha* segue sem assembly próprio, mas o assembly de *U. decumbens* cv. Basilisk (`GCA_964030465.3`) tem subgenoma rotulado por origem — cromossomos "b" = *brizantha* diploide (confirmado no artigo original, não assumido). BLASTP de `locus_001` contra o proteoma anotado achou `CAL5051155.1` (cromossomo `36b`, gene `URODEC1_LOCUS91967`) com **99,4% identidade (1 aa de diferença)** — a cópia real do subgenoma *brizantha*, com **UTRs reais anotadas** (mRNA 1.222 nt vs. CDS 525 nt) e estrutura de 4 éxons/3 íntrons confirmada de forma independente. Dois gargalos do §7 (7.2 gap espécie-proxy, 7.5 extensão do transcrito) rebaixados de "bloqueante" para "parcialmente resolvido" — a divergência que era desconhecida agora é um número baixo e conhecido, e existe UTR real pra usar no desenho de primers. Sequências em `brizantha_search/` (não versionado ainda — ver próximo passo). Ainda bloqueantes: validação experimental (§7.1) e off-target (§7.3). |
| 2026-07-13 | **Tabela de correlação locus_XXX → gene ID real, estendida às 6 loci confirmadas (§3.6, novo).** A pedido do usuário ("trazer o ID do gene em vez de locus"), estendi a metodologia do §3.5 (antes só `locus_001`) para `locus_002`, `005`, `009`, `012`, `015`. Verificado antes: o assembly-alvo `Embrapa_Uruz_1.0` (*U. ruziziensis*) não tem anotação gênica oficial no NCBI (sem `annotation_info` na API) — não existe "gene ID NCBI" nativo para nenhum locus na espécie-proxy, então o único ID de gene real disponível é via correlação com o subgenoma *brizantha* de *decumbens*. Achados: `locus_001` segue único caso em que a cópia b é também o melhor hit geral do proteoma; nos outros 5, o melhor hit geral cai numa cópia rd, com a cópia b às vezes bem mais divergente (`locus_005`: 91,6% vs. 100% da cópia rd — maior divergência entre as 6). `locus_009`/`015` convergem pra exatamente a mesma cópia b (`CAL4952393.1`), replicando por método independente o sinal de parologia já visto na filogenia. `locus_012` incluído só por rastreabilidade (já é MFT-like, não FT-like). Tabela completa em `brizantha_search/loci_brizantha_correlation.tsv`. |
